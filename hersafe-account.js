        ...
        const checkClerk = setInterval(function () {
            ...
            if (window.Clerk) {
                ...
                window.Clerk.load()
                    .then(function () {
                        clerkReady = true;
                        console.log("HerSafe Clerk account system loaded 💜");
                        resolve(window.Clerk);
                    })
                    .catch(function (error) {
                        console.error("HerSafe Clerk load error:", error);
                        reject(error);
                    });
            ...
            if (attempts >= maxAttempts) {
                clearInterval(checkClerk);
                reject(new Error("HerSafe could not find Clerk."));
            }
...
async function openHerSafeAuth(mode) {
    ...
    try {
        ...
        console.error("Clerk window error:", error);
        throw error;
    }
...
async function saveProgress(progress) {
    ...
    if (!clerk.user) {
        console.log("No HerSafe account is signed in.");
        return false;
    }

    try {
        const existing = clerk.user.unsafeMetadata || {};
        ...
        console.log("HerSafe quiz progress saved 💜");
        return true;
...
async function clearProgress() {
    ...
    const existing = clerk.user.unsafeMetadata || {};
    const cleaned = Object.assign({}, existing);
